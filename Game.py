...
class Game:
    ...
    Description:
    This class is the main controller of the SET game.
    It initializes all game systems, manages global state,
    and controls the main loop of the application.

    Parameters:
    None.

    Structure:
        - Initializes Pygame display, audio, and fonts
        - Creates all game screens and the table logic
        - Stores global game variables (scores, timers, settings)
        - Tracks the current active screen
        - Runs the main game loop

    Output:
        Launches the game window and continuously updates the game
        until the user exits.
    """
    def __init__(self):
        """Initialize display, game state, screens, and core components."""
    ...
    def run(self):
        """
        Description:
        Runs the main game loop and controls the flow of the application.

        Function:
        Continuously processes user input events, updates game logic,
        and calls the current screen to render the display until the
        user exits the game.
        """
        while self.running:
            # 1. Handle Events
