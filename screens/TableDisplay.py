...
class Display_card:
    ...
    Description:
    This class handles loading the card sprite sheet and extracting
    individual card images from it.

    Parameters:
    None.

    Structure:
        - Loads the full card image sheet
        - Uses card coordinates from the card deck data
        - Crops, scales, and rotates each card image when needed

    Output:
        Returns formatted card images that can be drawn on the screen.
    """
    def __init__(self):
        """Load the full sprite sheet and set transparency color."""
    ...
    def get_card_image(self, card_id):
        """
        Description:
        Extracts and formats one specific card image from the sprite sheet.

        Function:
        Uses the card ID to find its coordinates, crops the image from the
        sprite sheet, removes extra borders, scales it, rotates it, and
        returns the final card image.
        """
...
class Display_board:
    """
    README - Display_board

    Description:
    This class manages the visual display of cards on the game board.
    It positions cards in a grid, draws them on the screen, and detects
    which card the player clicks.

    Parameters:
    game:
        The main Game object used to access the table state and card display tools.

    Structure:
        - Stores references to the game table and card display system
        - Defines the board grid layout
        - Detects clicked cards
        - Draws cards and highlight effects

    Output:
        Draws the card board on the screen and returns clicked card indices when needed.
    """
    def __init__(self, game):
        """Initialize board layout settings and references to game objects."""
        self.game = game
    ...
    def get_clicked_card_index(self, mouse_pos):
        """Return the index of the clicked card, or None if no card was clicked."""
    ...
    def draw(self, screen):
        """
        Description:
        Draws all visible cards on the board with their current highlights.

        Function:
        Places each card in the correct grid position, draws selection or hint
        borders when needed, creates the white card background, and blits the
        final card image onto the screen.
        """
