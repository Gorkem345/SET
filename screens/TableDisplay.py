...
class Display_card:
    ...
    Description:
    This class loads the card sprite sheet and extracts individual
    card images based on their ID.

    Parameters:
    None.

    Structure:
        - Loads a single image containing all cards
        - Uses card coordinates from the deck dictionary
        - Applies cropping, scaling, and rotation

    Output:
        Returns processed card images ready for display.
    """
    def __init__(self):
        """Load the sprite sheet and set transparency color."""
    ...
    def get_card_image(self, card_id):
        """
        Description:
        Extracts and formats a single card image from the sprite sheet.

        Function:
        Uses the card ID to locate its position, crops unnecessary borders,
        scales the image, rotates it, and returns the final card surface.
        """
...
class Display_board:
    ...
    Description:
    This class manages the layout and rendering of cards on the board.
    It also detects which card the player clicks.

    Parameters:
    game:
        The main Game object used to access table data and card display.

    Structure:
        - Stores references to table and card display
        - Defines grid layout (4x3)
        - Handles click detection
        - Draws cards and highlights

    Output:
        Renders the board and returns clicked card indices when needed.
    """
    def __init__(self, game):
        """Initialize board layout and references to game data."""
        self.game = game
    ...
    def get_clicked_card_index(self, mouse_pos):
        """Return the index of the clicked card, or None if no card is clicked."""
    ...
    def draw(self, screen):
        """
        Description:
        Draws all cards on the board with selection and hint highlights.

        Function:
        Positions cards in a grid, draws highlight borders if selected
        or hinted, renders the card background, and blits the card image.
        """
